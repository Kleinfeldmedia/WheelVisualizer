...
class HomeController extends Controller
{
    ...
    {
        $Wheels = Wheel::select('brand', 'image', 'wheeldiameter', 'wheelwidth', 'style')->inRandomOrder()
            ->take(6)
            ->get();
        $Tires = Tire::select('prodbrand', 'prodimage', 'tiresize', 'price')->inRandomOrder()
            ->take(6)
            ->get();
        // dd($Tires);
        $years = Viflist::select('yr')->distinct('yr')
            ->orderBy('yr', 'Desc')
            ->limit(10)
            ->get();
    ...
    {
        $posts = Post::with('comments')->where('is_visible', 1)
            ->paginate(10);
    ...
    {
        $post = Post::with('comments')->where('is_visible', 1)
    ...
    public function checkorderstatus(Request $request)
    {
        ...
        try
        {
            ...
            $this->validate($request, ['email' => 'required', 'ordernumber' => 'required', ]);
            $order = Order::with('OrderItems')->where('email', $request->email)
            ...
            {
                return back()
                    ->with('flash_error', 'No Orders Found!!');
            }
        }
        catch(Exception $e)
        {
    ...
    public function wheels(Request $request)
    {
        try
        {
            ...
            if (isset($request->brand) && $request->brand) $Wheels = $Wheels->whereIn('brand', json_decode(base64_decode($request->brand)));

            if (isset($request->diameter) && $request->diameter) $Wheels = $Wheels->whereIn('wheeldiameter', json_decode(base64_decode($request->diameter)));

            if (isset($request->width) && $request->width) $Wheels = $Wheels->whereIn('wheelwidth', json_decode(base64_decode($request->width)));

            if (isset($request->search)) $Wheels = $Wheels->where('brand', 'LIKE', '%' . json_decode(base64_decode($request->search)) . '%');

            $Wheels = $Wheels->paginate(9);

            ///Brand with count
            $brands = Wheel::select('brand', \DB::raw('count(*) as total'))->groupBy('brand')
                ->get()
                ->sortBy('brand');

            ///wheeldiameter with count
            if (isset($request->brand) && $request->brand) $wheeldiameter = Wheel::select('wheeldiameter', \DB::raw('count(*) as total'))->whereIn('brand', json_decode(base64_decode($request->brand)))
                ->groupBy('wheeldiameter')
                ->get()
                ->sortBy('wheeldiameter');
            else $wheeldiameter = Wheel::select('wheeldiameter', \DB::raw('count(*) as total'))->groupBy('wheeldiameter')
                ->get()
                ->sortBy('wheeldiameter');

            ///wheelwidth with count
            if (isset($request->brand) && $request->brand) $wheelwidth = Wheel::select('wheelwidth', \DB::raw('count(*) as total'))->whereIn('brand', json_decode(base64_decode($request->brand)))
                ->groupBy('wheelwidth')
                ->get()
                ->sortBy('wheelwidth');
            else $wheelwidth = Wheel::select('wheelwidth', \DB::raw('count(*) as total'))->groupBy('wheelwidth')
                ->get()
                ->sortBy('wheelwidth');
            ...
            if (isset($request->car_id) && $request->car_id)
            {
                ...
                $car_images = CarImage::select('car_id', 'image', 'color_code')->wherecar_id($car_id)->where('image', 'LIKE', '%.png%')->with(['CarViflist' => function ($query)
                {
    ...
    public function vehicledetails(Request $request)
    {
        try
        {
            ...
            if (isset($request->make) && $request->changeBy == 'make' || $request->changeBy == '') $allData['year'] = $data = $viflist->select('yr')
                ->distinct('yr')
            ...
            // Make change  or Loading Filter
            if (isset($request->make) && isset($request->year) && $request->changeBy == 'year' || $request->changeBy == '') $allData['model'] = $data = $viflist->select('model')
                ->distinct('model')
                ->whereyr($request->year)
                ->wheremake($request->make)
                ->get();
            ...
            {
                $data = $viflist->select('vif', 'body', 'drs', 'whls')
                    ->whereyr($request->year)
                    ->wheremake($request->make)
                    ->wheremodel($request->model)
                    ->get()
                    ->unique('body', 'drs', 'whls')
                    ->toArray();
    ...
    public function wheelbrand(Request $request)
    {
        try
        {
            ...
            if (isset($request->car_id) && $request->car_id)
            {
                ...
                $car_images = CarImage::select('car_id', 'image', 'color_code')->wherecar_id($car_id)->with(['CarViflist' => function ($query)
                {
                    $query->select('vif', 'yr', 'make', 'model', 'body', 'drs', 'whls');
    ...
    public function selectCarByColor(Request $request)
    {
        ...
        {
            ///// Based On Car color
            $data = CarImage::select('image', 'car_id')->wherecar_id($request->vif)
                ->wherecolor_code($request->code)
                ->first();
            return response()
                ->json(['data' => $data]);
    ...
    public function tiredetailimages()
    {
        ...
        {
            if (!(strpos($value, 'Thumbs.db') !== false || strpos($value, '.xlsx') !== false || strpos($value, '.zip') !== false || strpos($value, '.pdf') !== false))
            {
                $path = str_replace("storage/tires/models/", "", $value);
                $folders = explode('/', $value);
                $lastPart = $folders[count($folders) - 1];

                Tire::where('benefitsimage1', 'like', '%' . $lastPart . '%')->update(['benefitsimage1' => $path, ]);

                Tire::where('benefitsimage2', 'like', '%' . $lastPart . '%')->update(['benefitsimage2' => $path, ]);

                Tire::where('benefitsimage3', 'like', '%' . $lastPart . '%')->update(['benefitsimage3' => $path, ]);

                Tire::where('benefitsimage4', 'like', '%' . $lastPart . '%')->update(['benefitsimage4' => $path, ]);
    ...
    function csv_vftp0032(Request $request)
    {
        ...
        $locationNames = array(
            ...
            '3046' => array(
                'TBC',
                'TBC3046',
                'TBC-Inv_SantaFeSpringsCA'
            ) ,
            '3050' => array(
                'TBC',
                'TBC3050',
                'TBC-Inv_GlendaleHeightsIL'
            ) ,
            '3058' => array(
                'TBC',
                'TBC3058',
                'TBC-Inv_LockbourneOH'
            ) ,
            '3060' => array(
                'TBC',
                'TBC3060',
            ...
            '3144' => array(
                'TBC',
                'TBC3144',
                'TBC-Inv_LibertyvilleIL'
            ) ,
            '3146' => array(
                'TBC',
                'TBC3146',
                'TBC-Inv_AlbuquerqueNM'
            ) ,
            '3149' => array(
                'TBC',
                'TBC3149',
            ...
            '3155' => array(
                'TBC',
                'TBC3155',
                'TBC-Inv_HicksvilleNY'
            ) ,
            '3159' => array(
                'TBC',
                'TBC3159',
                'TBC-Inv_SpringfieldMO'
            ) ,
            '3163' => array(
                'TBC',
                'TBC3163',
                'TBC-Inv_ChatsworthCA'
            ) ,
            '3165' => array(
                'TBC',
                'TBC3165',
        ...
        if (($inpfile = fopen($filepath, 'r')) !== false)
        {
            ...
            while (($data = fgetcsv($inpfile, 10000)) !== false)
            {
                ...
                if ($flag != 0)
                {
                    ...
                    {
                        \DB::table('inventories_test')
                            ->insert(['filename' => $filename, 'partno' => $data[0] ? $data[0] : null, 'vendor_partno' => $data[1] ? $data[1] : null, 'mpn' => $data[2] ? $data[2] : null, 'description' => $data[3] ? $data[3] : null, 'brand' => $data[4] ? $data[4] : null, 'model' => $data[5] ? $data[5] : null, 'location_code' => $data[6] ? $data[6] : null, 'available_qty' => $data[7] ? $data[7] : null, 'price' => $data[8] ? $data[8] : null, 'drop_shipper' => $data[9] ? $data[9] : null, 'ds_vendor_code' => $data[10] ? $data[10] : null, 'location_name' => $data[11] ? $data[11] : null, ]);
                        // $inventory->partno = $data[0];
                        // $inventory->vendor_partno = $data[1];
                        // $inventory->mpn = $data[2];
                        // $inventory->description = $data[3];
    ...
    public function addStarRating(Request $request)
    {
        ...
        try
        {
            ...
            return back()
                ->with('success', 'Thanks for Your Review & Ratings!!');
    ...
    public function VerifyWheelProductImages(Request $request)
    {
        ...
        foreach ($products as $key => $product)
        {
            ...
            {
                if (!front_back_filecheck($product
                    ->wheel
                    ->image))
                {
                    $arr[$product
